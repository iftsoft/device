...
func (cfg *DeviceConfig) String() string {
	...
	str := fmt.Sprintf("\nDevice \"%s\" config: %s %s %s %s %s %s %s %s",
		cfg.DevName, cfg.Linker, cfg.Common, cfg.Printer, cfg.Reader,
...
func (cfg DeviceConfigList) String() string {
	if cfg == nil {
		return ""
	}
	str := "\nDevices:"
...
func GetDefaultDeviceConfigList() DeviceConfigList {
	return make(DeviceConfigList, 0)
}
